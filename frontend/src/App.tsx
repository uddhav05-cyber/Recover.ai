...
export default function App() {
  ...
  const logout = () => {
    ...
    sessionStorage.removeItem("recoverai_email");
    setLoggedIn(false);
  };
  ...
  return <main className="min-h-screen bg-[#f4f1ea] text-[#1c2624]"><div className="mx-auto max-w-7xl px-5 py-7 lg:px-10"><header className="flex flex-wrap items-end justify-between gap-5 border-b border-[#c9c6bc] pb-7"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.25em] text-[#8a5a32]">Revenue operations / live view</p><h1 className="font-serif text-5xl leading-none">RecoverAI</h1></div><div className="flex items-center gap-4"><div className="text-right text-sm text-[#65706a]"><p>{email}</p><p className="mt-2 flex items-center justify-end gap-2 text-xs uppercase tracking-widest"><span className="h-2 w-2 rounded-full bg-[#4e8b70]" /> monitoring</p></div><div className="flex gap-2 border-l border-[#c9c6bc] pl-4"><button aria-label="Refresh dashboard" title="Refresh dashboard" onClick={() => window.location.reload()} className="border border-[#c9c6bc] px-3 py-2 text-sm text-[#65706a] hover:border-[#367057] hover:text-[#367057]" type="button">↻</button><button onClick={logout} className="border border-[#c9c6bc] px-3 py-2 text-xs font-bold uppercase tracking-widest text-[#65706a] hover:border-[#a44635] hover:text-[#a44635]" type="button">Sign out</button></div></div></header>
...
function Status({ status }: { status: string }) { const normalized = status.toLowerCase(); const tone = normalized.includes("recovered") || normalized.includes("active") ? "bg-[#e3f0e8] text-[#367057]" : normalized.includes("risk") || normalized.includes("escalated") || normalized.includes("failed") ? "bg-[#f9e6df] text-[#a44635]" : "bg-[#ecebe5] text-[#65706a]"; return <span className={`inline-flex max-w-full items-center px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${tone}`}>{status.replaceAll("_", " ")}</span>; }
